import { AuthMethod, AuthPayload, AuthResult, InvalidAuthPayloadError } from './AuthMethod'
...
function canonicalPhoneNumber(payload: AuthPayload): string {
  ...
  if (!E164_PHONE_NUMBER.test(phoneNumber)) {
    throw new InvalidAuthPayloadError('phoneNumber must use canonical E.164 format.')
  }
  return phoneNumber
}
...
export class TwilioAuthMethod extends AuthMethod {
  public readonly methodType = 'TwilioPhone'
