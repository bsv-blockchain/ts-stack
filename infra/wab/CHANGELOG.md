...
- Restore the prior deployment's browser compatibility for
  `X-Correlation-ID` and future well-formed request headers while preserving an
  explicit `WAB_CORS_ALLOWED_HEADERS` value as a strict operator allowlist.
