...
type RouteMethod = 'get' | 'put' | 'post' | 'patch' | 'delete'

const preAuthRateLimit = rateLimit(
  rateLimitOptions('UHRP_PRE_AUTH_RATE_LIMIT', { windowMs: 60_000, limit: 300 })
)
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.json = (json: any) => {
    log.info(
      { operation: 'response.json', method: req.method, url: req.url },
...
preAuthRoutes
  .filter(route => (route as any).unsecured)
  .forEach(route => {
    log.info(
      { operation: 'route.register', phase: 'pre_auth', secured: false, route_path: route.path },
      'Registering route'
    )
    // If we need middleware for a route, attach it
    if ((route as any).middleware) {
      app[route.type as RouteMethod](route.path, (route as any).middleware, (route as any).func)
    } else {
      app[route.type as RouteMethod](route.path, (route as any).func)
    }
  })
...
preAuthRoutes
  ...
  .forEach(route => {
    log.info(
      { operation: 'route.register', phase: 'pre_auth', secured: true, route_path: route.path },
      'Registering route'
    )
    // If we need middleware for a route, attach it
    if ((route as any).middleware) {
...
;(async () => {
  ...
  const paymentMiddleware = createPaymentMiddleware({
    ...
    calculateRequestPrice: async req => {
      if (req.url === '/upload') {
        ...
        try {
          const satoshis = await getPriceForFile({
            fileSize: +fileSize,
  ...
  postAuthRoutes.forEach(route => {
    log.info(
      { operation: 'route.register', phase: 'post_auth', secured: true, route_path: route.path },
      'Registering route'
    )
    // If we need middleware for a route, attach it
    if ((route as any).middleware) {
  ...
  const server = app.listen(HTTP_PORT, () => {
    const identityKey = PrivateKey.fromString(SERVER_PRIVATE_KEY).toPublicKey().toString()
    log.info(
  ...
  log.error(
    { operation: 'bootstrap', outcome: 'error', err: error },
    'UHRP Storage Server failed to start'
  )
  process.exit(1)
})
