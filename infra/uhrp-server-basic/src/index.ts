...
preAuthRoutes
  ...
  .forEach(route => {
    log.info(
      {
        operation: 'route.register',
        phase: 'pre_auth_unsecured',
...
;(async () => {
  ...
  app.use((req, res) => {
    log.info(
      { operation: 'route.not_found', method: req.method, path: req.path },
      'Route not found'
    )
    res.status(404).json({
      status: 'error',
  ...
  const server = app.listen(HTTP_PORT, () => {
    const idKey = PrivateKey.fromString(SERVER_PRIVATE_KEY).toPublicKey().toString()
    log.info(
