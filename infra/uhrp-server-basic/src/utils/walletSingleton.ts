...
type WalletSignerKeyDeriver = ConstructorParameters<typeof WalletSigner>[1]
...
export async function getWallet(): Promise<WalletInterface> {
    if (!walletInstance) {
        ...
        const signer = new WalletSigner(chain, keyDeriver as unknown as WalletSignerKeyDeriver, storageManager);
