...
export function normalizeUhrpPagination(
  ...
  if (limitValue == null) {
    limit = defaultLimit === -1 ? Number.MAX_SAFE_INTEGER : defaultLimit
  }
  const offset = offsetValue == null ? 0 : Number(offsetValue)
  if (!Number.isSafeInteger(limit) || limit < 1 || (maxLimit !== -1 && limit > maxLimit)) {
    const maximum = maxLimit === -1 ? '' : ` no greater than ${maxLimit}`
    throw new RangeError(`limit must be a positive integer${maximum}`)
  }
  if (!Number.isSafeInteger(offset) || offset < 0 || (maxOffset !== -1 && offset > maxOffset)) {
    const maximum = maxOffset === -1 ? '' : ` no greater than ${maxOffset}`
    throw new RangeError(`offset must be a non-negative integer${maximum}`)
  }
  return { limit, offset }
}
