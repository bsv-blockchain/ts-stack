...
export function createSendMessageRoute (
  ctx: Pick<MessageBoxContext, 'knex' | 'wallet'>
): SendMessageRoute {
  const { knex, wallet } = ctx
  ...
  return {
    ...
    func: async (req: SendMessageRequest, res: Response): Promise<Response> => {
      ...
      try {
        ...
        const deliveryFeeOnce = await getServerDeliveryFee(knex, boxType)

        type FeeRow = { recipient: string, recipientFee: number, allowed: boolean, blockedReason?: string }
        const feeRows: FeeRow[] = []
        for (const r of recipientsTrimmed) {
          const rf = await getRecipientFee(knex, r, senderKey, boxType)
        ...
        for (const { recipient: r } of feeRows) {
          ...
          try {
            if (shouldUseFCMDelivery(boxType)) {
              await sendFCMNotification(knex, r, { title: 'New Message', messageId: perRecipientMessageId })
            }
          } catch (deliveryError) {
