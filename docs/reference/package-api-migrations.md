...
| `@bsv/overlay-topics`             | `1.6.1`      | `1.6.10` | patch     | [API and usage](../packages/overlays/overlay-topics.md)               | No consumer migration is required; topic IDs, lookup contracts, and persisted formats are unchanged. Operators of collections that already contain duplicate rows violating a unique index should remove the duplicates so the index can finish building; lookups now succeed either way, and the index is built on a later call without a restart.                                                                                                                                                      |
...
- Release note: Keeps Mongo-backed lookups serving when an index build fails, retries index creation after a failed build instead of caching the rejection, and upserts outpoint-keyed records so repeated admission of the same output cannot create duplicate rows. Applies to every topic storage manager through a shared CollectionIndexes helper.
- Migration: No consumer migration is required; topic IDs, lookup contracts, and persisted formats are unchanged. Operators of collections that already contain duplicate rows violating a unique index should remove the duplicates so the index can finish building; lookups now succeed either way, and the index is built on a later call without a restart.
