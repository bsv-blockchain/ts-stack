...
- Release note: Adds WAB-administered UMP ambiguity pinning, verified same-or-new phone-number changes that roll the presentation key by spending the current UMP token, and retry-safe WAB commit semantics, alongside the local-first ChainTracks management, proof-of-work enforcement, bounded validation, cache, and observability candidate.
...
- Release note: Carries the browser-safe WAB UMP-pin fallback and verified phone-number/presentation-key rotation APIs together with the local-first ChainTracks controller, proof-of-work enforcement, bounded fetch and inline validation, checkpoint cache, download-budget contracts, and lockstep Wallet Toolbox corrections.
- Migration: Existing browser exports, defaults, and authentication calls remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must save the returned manager snapshot after success. Deploy compatible WAB and UMP overlay services first.
...
- Release note: Carries the mobile-safe WAB UMP-pin fallback and verified phone-number/presentation-key rotation APIs together with the local-first ChainTracks controller, proof-of-work enforcement, bounded fetch and inline validation, checkpoint cache, download-budget contracts, and lockstep Wallet Toolbox corrections.
