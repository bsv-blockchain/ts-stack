...
| `@bsv/wallet-toolbox`             | `2.9.0`      | `2.10.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | Existing authentication and storage APIs remain compatible. WAB operators should deploy the additive auth response plus start, complete, commit, and finalize phone-change endpoints before exposing client UI. Wallet applications call startPhoneNumberChange, collect the SMS code, call completePhoneNumberChange, and persist saveSnapshot immediately after success; interrupted transitions recover during verified login. A pin is considered only when normal verified lineage resolution remains ambiguous and only when it names a verified lookup candidate. Declared inputs and their complete dependency proofs are validated as before, while structurally valid unrelated BEEF branches are ignored. Storage replication now commits each received page and its sync checkpoint in one provider transaction; failed pages roll back and retry from the prior durable checkpoint without a wire-format or persisted-schema migration. |
| `@bsv/wallet-toolbox-client`      | `2.9.0`      | `2.10.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-client.md)          | Existing browser exports, defaults, authentication calls, storage wire contracts, and persisted schemas remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must save the returned manager snapshot after success. Deploy WAB with the staged commit/finalize protocol and compatible UMP overlay services first. Storage replication automatically commits page data and its checkpoint together; no consumer migration is required.                                                                                                                                                                                                                                                                                                                                                                                                                |
| `@bsv/wallet-toolbox-mobile`      | `2.9.0`      | `2.10.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-mobile.md)          | Existing React Native exports, remote-storage behavior, authentication calls, storage wire contracts, and persisted schemas remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must persist saveSnapshot after success. Deploy WAB with the staged commit/finalize protocol and compatible UMP overlay services first. Storage replication automatically commits page data and its checkpoint together; no consumer migration is required.                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- Release note: Adds an engine lookup-result cardinality ceiling before transaction/proof hydration and an optional TopicManager abort hook for provisional admission state when strict transaction broadcast fails.
...
- Release note: Adds WAB-administered UMP ambiguity pinning, interruption-safe same-or-new phone-number changes that stage the WAB key before spending the current UMP token and finalize afterward, restart recovery across current and pending keys, inputBEEF pruning to declared-input proof dependencies, and atomic page-level wallet storage replication, alongside the local-first ChainTracks management, proof-of-work enforcement, bounded validation, cache, and observability candidate.
- Migration: Existing authentication and storage APIs remain compatible. WAB operators should deploy the additive auth response plus start, complete, commit, and finalize phone-change endpoints before exposing client UI. Wallet applications call startPhoneNumberChange, collect the SMS code, call completePhoneNumberChange, and persist saveSnapshot immediately after success; interrupted transitions recover during verified login. A pin is considered only when normal verified lineage resolution remains ambiguous and only when it names a verified lookup candidate. Declared inputs and their complete dependency proofs are validated as before, while structurally valid unrelated BEEF branches are ignored. Storage replication now commits each received page and its sync checkpoint in one provider transaction; failed pages roll back and retry from the prior durable checkpoint without a wire-format or persisted-schema migration.
...
- Release note: Carries the browser-safe WAB UMP-pin fallback, interruption-safe staged phone-number/presentation-key rotation APIs with verified-login recovery, inputBEEF pruning to declared-input dependencies, and atomic page-level IndexedDB replication together with the local-first ChainTracks controller, proof-of-work enforcement, bounded fetch and inline validation, checkpoint cache, download-budget contracts, and lockstep Wallet Toolbox corrections.
- Migration: Existing browser exports, defaults, authentication calls, storage wire contracts, and persisted schemas remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must save the returned manager snapshot after success. Deploy WAB with the staged commit/finalize protocol and compatible UMP overlay services first. Storage replication automatically commits page data and its checkpoint together; no consumer migration is required.
...
- Release note: Carries the mobile-safe WAB UMP-pin fallback, interruption-safe staged phone-number/presentation-key rotation APIs with verified-login recovery, inputBEEF pruning to declared-input dependencies, and atomic page-level storage replication together with the local-first ChainTracks controller, proof-of-work enforcement, bounded fetch and inline validation, checkpoint cache, download-budget contracts, and lockstep Wallet Toolbox corrections.
- Migration: Existing React Native exports, remote-storage behavior, authentication calls, storage wire contracts, and persisted schemas remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must persist saveSnapshot after success. Deploy WAB with the staged commit/finalize protocol and compatible UMP overlay services first. Storage replication automatically commits page data and its checkpoint together; no consumer migration is required.
