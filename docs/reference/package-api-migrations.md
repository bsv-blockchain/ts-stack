...
| `@bsv/wallet-toolbox`             | `2.7.0`      | `2.7.1` | patch     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | No consumer migration is required. Proof polling and lifecycle diagnostics may continue updating pending rows, but can no longer postpone the scheduled terminal review. A child whose local parent is invalid or double-spent is failed atomically; failed-parent outputs stay quarantined while independent inputs remain reusable. |
| `@bsv/wallet-toolbox-client`      | `2.7.0`      | `2.7.1` | patch     | [API and usage](../packages/wallet/wallet-toolbox-client.md)          | No consumer migration is required. Browser exports, runtime defaults, and wire contracts are unchanged; local browser storage using the Monitor receives the same safe pending-chain cleanup behavior.                                                                                                                                |
| `@bsv/wallet-toolbox-mobile`      | `2.7.0`      | `2.7.1` | patch     | [API and usage](../packages/wallet/wallet-toolbox-mobile.md)          | No consumer migration is required. React Native exports, runtime defaults, remote-storage behavior, and wire contracts are unchanged; shared reconciliation behavior remains aligned with the parent package.                                                                                                                         |
...
- Release note: Makes proactive pending-transaction reconciliation age requests from immutable creation time and retires descendants of locally terminal parents without sacrificing unrelated valid inputs.
- Migration: No consumer migration is required. Proof polling and lifecycle diagnostics may continue updating pending rows, but can no longer postpone the scheduled terminal review. A child whose local parent is invalid or double-spent is failed atomically; failed-parent outputs stay quarantined while independent inputs remain reusable.
...
- Release note: Carries the lockstep browser build with the Wallet Toolbox creation-age and failed-parent pending-reconciliation correction.
- Migration: No consumer migration is required. Browser exports, runtime defaults, and wire contracts are unchanged; local browser storage using the Monitor receives the same safe pending-chain cleanup behavior.
...
- Release note: Carries the lockstep mobile build with the Wallet Toolbox creation-age and failed-parent pending-reconciliation correction.
- Migration: No consumer migration is required. React Native exports, runtime defaults, remote-storage behavior, and wire contracts are unchanged; shared reconciliation behavior remains aligned with the parent package.
