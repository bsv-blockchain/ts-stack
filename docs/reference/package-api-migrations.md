...
- Release note: Prevents reuse or coalescing of spending approvals, paginates complete spending history, requires HTTPS for remote wallet storage and credential-bearing Arcade SSE outside loopback development, prevents SSE dependency debug logs from exposing credentials, and documents snapshots as wallet-equivalent secrets. Retains the existing self-contained snapshot API and formats plus BRC-95, BRC-100, and canonical AtomicBEEF compatibility.
...
- Release note: Carries the Wallet Toolbox spending-authorization, complete-accounting, remote-storage and Arcade SSE transport hardening, and SSE credential-log prevention into the browser package while documenting snapshots as wallet-equivalent secrets and preserving snapshot, browser-export, and byte-boundary contracts.
...
- Release note: Carries the Wallet Toolbox spending-authorization, complete-accounting, remote-storage and Arcade SSE transport hardening, and SSE credential-log prevention into the React Native package while documenting snapshots as wallet-equivalent secrets and preserving snapshot, mobile-export, and byte-boundary contracts.
