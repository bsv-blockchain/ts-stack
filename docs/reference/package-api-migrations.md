...
| `@bsv/wallet-toolbox`             | `2.6.1`      | `2.6.2` | patch     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | No consumer migration is required. Existing JSON bridges regain the historical numeric-array response shape; AtomicBEEF consumers and binary Wallet Wire transports continue to support typed byte arrays. Omit WALLET_STORAGE_CORS_ALLOWED_HEADERS for additive well-formed preflight headers, or configure an exact list for a strict browser policy. |
...
- Release note: Restores completed createAction and signAction Atomic BEEF results to JSON-safe numeric arrays and preserves forward-compatible Storage Server browser preflights unless the operator configures a strict request-header allowlist.
- Migration: No consumer migration is required. Existing JSON bridges regain the historical numeric-array response shape; AtomicBEEF consumers and binary Wallet Wire transports continue to support typed byte arrays. Omit WALLET_STORAGE_CORS_ALLOWED_HEADERS for additive well-formed preflight headers, or configure an exact list for a strict browser policy.
