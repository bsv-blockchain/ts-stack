...
| `@bsv/wallet-toolbox`             | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | Mainnet/testnet browser and webview runtimes temporarily use the legacy CORS-enabled ChainTracks service; Node runtimes retain Arcade/go-chaintracks v2, and getHeight falls back to WhatsOnChain on supported networks. TTN now uses https://arcade-v2-ttn-us-1.bsvblockchain.tech unless callers override or disable it. Unrelated actions remain outside open workspaces; providers may advertise resume support and a reservation limit, default 256. Direct invalid-change release now throws machine-checkable WERR_UTXO_REVIEW_INCONCLUSIVE and changes nothing when any verdict is unknown. Monitor Admin scans in bounded pages and, after explicit confirmation, releases only confirmed-spent outputs while retaining/reporting unknowns. A scheduled monitor pass applies durable Arcade terminal status missed by SSE; unknown provider results never mutate state. |
| `@bsv/wallet-toolbox-client`      | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-client.md)          | Mainnet/testnet browser and webview runtimes temporarily select the legacy CORS-enabled ChainTracks service and getHeight can fall back to WhatsOnChain; Node behavior is unaffected. TTN wallets receive isolated resolver and Arcade defaults. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `@bsv/wallet-toolbox-mobile`      | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-mobile.md)          | Existing native mobile consumers are unchanged; webview runtimes follow the browser ChainTracks selection. TTN wallets receive isolated resolver and Arcade defaults. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- Release note: Routes TTN wallets through the isolated teratestnet preset and public Arcade provider, isolates/resumes bounded action batches, keeps browser ChainTracks CORS-reachable, proactively reconciles missed Arcade terminal verdicts, and makes invalid-change review tri-state, paged, and fail safe without an explorer.
- Migration: Mainnet/testnet browser and webview runtimes temporarily use the legacy CORS-enabled ChainTracks service; Node runtimes retain Arcade/go-chaintracks v2, and getHeight falls back to WhatsOnChain on supported networks. TTN now uses https://arcade-v2-ttn-us-1.bsvblockchain.tech unless callers override or disable it. Unrelated actions remain outside open workspaces; providers may advertise resume support and a reservation limit, default 256. Direct invalid-change release now throws machine-checkable WERR_UTXO_REVIEW_INCONCLUSIVE and changes nothing when any verdict is unknown. Monitor Admin scans in bounded pages and, after explicit confirmation, releases only confirmed-spent outputs while retaining/reporting unknowns. A scheduled monitor pass applies durable Arcade terminal status missed by SSE; unknown provider results never mutate state.
...
- Release note: Carries the lockstep browser build with isolated TTN/Arcade routing, bounded action-batch recovery, a CORS-reachable mainnet/testnet ChainTracks default, proactive Arcade rejection reconciliation, and paged tri-state UTXO review.
- Migration: Mainnet/testnet browser and webview runtimes temporarily select the legacy CORS-enabled ChainTracks service and getHeight can fall back to WhatsOnChain; Node behavior is unaffected. TTN wallets receive isolated resolver and Arcade defaults. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.
...
- Release note: Carries the lockstep mobile build with isolated TTN/Arcade routing, bounded action-batch recovery, proactive Arcade rejection reconciliation, and paged tri-state UTXO review.
- Migration: Existing native mobile consumers are unchanged; webview runtimes follow the browser ChainTracks selection. TTN wallets receive isolated resolver and Arcade defaults. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.
