...
| `@bsv/wallet-toolbox`             | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | Mainnet/testnet browser and webview runtimes temporarily use the legacy CORS-enabled ChainTracks service; Node retains Arcade/go-chaintracks v2, and getHeight falls back to WhatsOnChain on supported networks. TTN receives its isolated resolver and Arcade default. Exact untouched default baskets at 144 outputs / 32 satoshis advance to a 5,000-satoshi preference through future authorized actions; custom values remain authoritative. Direct invalid-change release throws machine-checkable WERR_UTXO_REVIEW_INCONCLUSIVE and changes nothing when any verdict is unknown. Monitor Admin reviews bounded pages and may explicitly release only confirmed-spent outputs while retaining/reporting unknowns. A scheduled Monitor pass applies durable Arcade terminal status missed by SSE; unknown provider results never mutate state. |
| `@bsv/wallet-toolbox-client`      | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-client.md)          | Mainnet/testnet browser and webview runtimes temporarily select the legacy CORS-enabled ChainTracks service and getHeight can fall back to WhatsOnChain; Node behavior is unaffected. TTN wallets receive isolated resolver and Arcade defaults. Exact untouched IndexedDB defaults advance from 32 to 5,000 satoshis through ordinary authorized actions; custom values and remote-storage wire contracts remain unchanged. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.                                                                                                                                                                                                                                           |
| `@bsv/wallet-toolbox-mobile`      | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-mobile.md)          | Existing native mobile consumers are unchanged; webview runtimes follow the browser ChainTracks selection. TTN wallets receive isolated resolver and Arcade defaults. Wallets progressively adopt useful change values through future authorized actions; custom values, BRC-100 methods, and remote-storage wire contracts remain unchanged. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.                                                                                                                                                                                                                                                                                                                          |
...
- Release note: Adds isolated TTN routing and default Arcade support, isolated resumable action batches, progressive managed-change liquidity, browser-safe ChainTracks defaults, proactive Arcade terminal reconciliation, and paged tri-state invalid-change review.
- Migration: Mainnet/testnet browser and webview runtimes temporarily use the legacy CORS-enabled ChainTracks service; Node retains Arcade/go-chaintracks v2, and getHeight falls back to WhatsOnChain on supported networks. TTN receives its isolated resolver and Arcade default. Exact untouched default baskets at 144 outputs / 32 satoshis advance to a 5,000-satoshi preference through future authorized actions; custom values remain authoritative. Direct invalid-change release throws machine-checkable WERR_UTXO_REVIEW_INCONCLUSIVE and changes nothing when any verdict is unknown. Monitor Admin reviews bounded pages and may explicitly release only confirmed-spent outputs while retaining/reporting unknowns. A scheduled Monitor pass applies durable Arcade terminal status missed by SSE; unknown provider results never mutate state.
...
- Release note: Carries the lockstep browser build with isolated TTN/Arcade routing, resumable action batches, progressive IndexedDB managed-change liquidity, a CORS-reachable ChainTracks default, proactive Arcade rejection reconciliation, and paged tri-state UTXO review.
- Migration: Mainnet/testnet browser and webview runtimes temporarily select the legacy CORS-enabled ChainTracks service and getHeight can fall back to WhatsOnChain; Node behavior is unaffected. TTN wallets receive isolated resolver and Arcade defaults. Exact untouched IndexedDB defaults advance from 32 to 5,000 satoshis through ordinary authorized actions; custom values and remote-storage wire contracts remain unchanged. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.
...
- Release note: Carries the lockstep mobile build with isolated TTN/Arcade routing, resumable action batches, progressive managed-change liquidity, proactive Arcade rejection reconciliation, and paged tri-state UTXO review.
- Migration: Existing native mobile consumers are unchanged; webview runtimes follow the browser ChainTracks selection. TTN wallets receive isolated resolver and Arcade defaults. Wallets progressively adopt useful change values through future authorized actions; custom values, BRC-100 methods, and remote-storage wire contracts remain unchanged. Direct inconclusive release throws WERR_UTXO_REVIEW_INCONCLUSIVE without mutation; authenticated Monitor Admin can explicitly release conclusive spent subsets page by page.
