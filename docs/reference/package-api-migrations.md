...
- Release note: Rebuilds BRC-29 payment transport against the SDK correction for binary Wallet Wire transaction results and recovers pending typed-array tokens serialized through JSON.
- Migration: No API migration is required. Upgrade @bsv/sdk and @bsv/message-box-client together; historical number-array wallets and current Uint8Array Wallet Wire substrates produce the same JSON-safe payment token, while pending numeric-key tokens remain receivable.
