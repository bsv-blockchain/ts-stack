...
- Release note: Isolates action batches by explicit transaction-graph membership, adds bounded exact-input lease resume, and makes Arcade rejection and proof reconciliation durable even when no explorer is configured.
...
- Release note: Carries the lockstep browser build with explicit action-batch isolation, bounded lease recovery, and conservative Arcade rejection reconciliation.
- Migration: No browser consumer migration is required. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured.
...
- Release note: Carries the lockstep mobile build with explicit action-batch isolation, bounded lease recovery, and conservative Arcade rejection reconciliation.
- Migration: No mobile consumer migration is required. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured.
