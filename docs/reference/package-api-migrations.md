...
| `@bsv/wallet-toolbox`             | `2.9.0`      | `2.10.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | Existing authentication calls remain compatible. WAB operators should deploy the additive auth response and phone-change endpoints before exposing client UI. Wallet applications that offer phone changes call startPhoneNumberChange, collect the SMS code, call completePhoneNumberChange, and persist saveSnapshot immediately after success. A pin is considered only when normal verified lineage resolution remains ambiguous and only when it names a verified lookup candidate. Declared inputs and their complete dependency proofs are validated as before, while structurally valid unrelated BEEF branches are ignored. |
| `@bsv/wallet-toolbox-client`      | `2.9.0`      | `2.10.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-client.md)          | Existing browser exports, defaults, authentication calls, and wire contracts remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must save the returned manager snapshot after success. Deploy compatible WAB and UMP overlay services first.                                                                                                                                                                                                                                                                                        |
| `@bsv/wallet-toolbox-mobile`      | `2.9.0`      | `2.10.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-mobile.md)          | Existing React Native exports, remote-storage behavior, authentication calls, and wire contracts remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must persist saveSnapshot after success. Deploy compatible WAB and UMP overlay services first.                                                                                                                                                                                                                                                                                  |
...
- Release note: Adds persistent first-writer reservations for UMP presentation and recovery hashes, transfers a reservation only when the owning outpoint is consumed, exposes reservation-store integration for multi-replica overlays, and returns every matching legacy UMP candidate so wallet clients can apply verified lineage or an operator pin. It also retains the collection-index resilience and opt-in repair controls from the prior candidate.
...
- Release note: Adds WAB-administered UMP ambiguity pinning, verified same-or-new phone-number changes that roll the presentation key by spending the current UMP token, retry-safe WAB commit semantics, and inputBEEF pruning to declared-input proof dependencies, alongside the local-first ChainTracks management, proof-of-work enforcement, bounded validation, cache, and observability candidate.
- Migration: Existing authentication calls remain compatible. WAB operators should deploy the additive auth response and phone-change endpoints before exposing client UI. Wallet applications that offer phone changes call startPhoneNumberChange, collect the SMS code, call completePhoneNumberChange, and persist saveSnapshot immediately after success. A pin is considered only when normal verified lineage resolution remains ambiguous and only when it names a verified lookup candidate. Declared inputs and their complete dependency proofs are validated as before, while structurally valid unrelated BEEF branches are ignored.
...
- Release note: Carries the browser-safe WAB UMP-pin fallback, verified phone-number/presentation-key rotation APIs, and inputBEEF pruning to declared-input dependencies together with the local-first ChainTracks controller, proof-of-work enforcement, bounded fetch and inline validation, checkpoint cache, download-budget contracts, and lockstep Wallet Toolbox corrections.
- Migration: Existing browser exports, defaults, authentication calls, and wire contracts remain compatible. A settings UI may call startPhoneNumberChange and completePhoneNumberChange, including with the currently registered number, and must save the returned manager snapshot after success. Deploy compatible WAB and UMP overlay services first.
...
- Release note: Carries the mobile-safe WAB UMP-pin fallback, verified phone-number/presentation-key rotation APIs, and inputBEEF pruning to declared-input dependencies together with the local-first ChainTracks controller, proof-of-work enforcement, bounded fetch and inline validation, checkpoint cache, download-budget contracts, and lockstep Wallet Toolbox corrections.
