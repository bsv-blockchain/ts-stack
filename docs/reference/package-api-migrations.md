...
| `@bsv/wallet-toolbox`             | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | Mainnet, testnet, STN, TSTN, and mock defaults are unchanged. TTN now uses https://arcade-v2-ttn-us-1.bsvblockchain.tech unless callers pass an explicit Arcade URL or an empty string to disable it. Unrelated actions continue through the legacy path instead of joining or committing an open workspace. Providers may optionally advertise resume support and a maximum reservation count; built-in providers default to 256 outputs and operators can select another positive value or -1 for unlimited operation. Arcade operators should preserve SSE status codes and extraInfo so terminal input conflicts can be quarantined immediately. Invalid-change review rejects an inconclusive batch without changing outputs; Monitor Admin scans by default and operators must explicitly select release for outputs positively confirmed spent. |
| `@bsv/wallet-toolbox-client`      | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-client.md)          | Existing browser consumers are unchanged. Browser wallets configured for chain ttn receive the TTN resolver and Arcade defaults automatically. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews reject without changing outputs.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `@bsv/wallet-toolbox-mobile`      | `2.6.5`      | `2.7.0` | minor     | [API and usage](../packages/wallet/wallet-toolbox-mobile.md)          | Existing mobile consumers are unchanged. Mobile wallets configured for chain ttn receive the TTN resolver and Arcade defaults automatically. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews reject without changing outputs.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- Release note: Routes TTN wallet overlay operations through the isolated teratestnet preset, enables the public TTN Arcade provider, isolates action batches with bounded lease resume, and makes Arcade rejection plus invalid-change reconciliation fail safe without an explorer.
- Migration: Mainnet, testnet, STN, TSTN, and mock defaults are unchanged. TTN now uses https://arcade-v2-ttn-us-1.bsvblockchain.tech unless callers pass an explicit Arcade URL or an empty string to disable it. Unrelated actions continue through the legacy path instead of joining or committing an open workspace. Providers may optionally advertise resume support and a maximum reservation count; built-in providers default to 256 outputs and operators can select another positive value or -1 for unlimited operation. Arcade operators should preserve SSE status codes and extraInfo so terminal input conflicts can be quarantined immediately. Invalid-change review rejects an inconclusive batch without changing outputs; Monitor Admin scans by default and operators must explicitly select release for outputs positively confirmed spent.
...
- Release note: Carries the lockstep browser build with isolated TTN routing and its default public Arcade provider, explicit action-batch isolation, bounded lease recovery, conservative Arcade rejection reconciliation, and fail-safe invalid-change review.
- Migration: Existing browser consumers are unchanged. Browser wallets configured for chain ttn receive the TTN resolver and Arcade defaults automatically. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews reject without changing outputs.
...
- Release note: Carries the lockstep mobile build with isolated TTN routing and its default public Arcade provider, explicit action-batch isolation, bounded lease recovery, conservative Arcade rejection reconciliation, and fail-safe invalid-change review.
- Migration: Existing mobile consumers are unchanged. Mobile wallets configured for chain ttn receive the TTN resolver and Arcade defaults automatically. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews reject without changing outputs.
