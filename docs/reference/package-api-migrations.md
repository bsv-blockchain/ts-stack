...
| `@bsv/message-box-client`         | `2.2.2`      | `2.3.0` | minor     | [API and usage](../packages/messaging/message-box-client.md)          | Existing listMessages and listMessagesLite calls continue to fetch all available messages. Applications that need an aggregate memory ceiling should set limit and/or maxPages; no additional BRC-105 approval callback is required because AuthFetch uses wallet permissions.                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `@bsv/sdk`                        | `2.3.1`      | `2.3.2` | patch     | [API and usage](../packages/sdk/bsv-sdk.md)                           | No consumer migration is required. Existing HTTPWalletWire constructor calls, injected HTTP clients, Wallet Wire messages, and substrate selection order remain compatible.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `@bsv/wallet-toolbox`             | `2.6.5`      | `2.6.6` | patch     | [API and usage](../packages/wallet/wallet-toolbox.md)                 | No consumer migration is required. Unrelated actions continue through the legacy path instead of joining or committing an open workspace. Providers may optionally advertise resume support and a maximum reservation count; built-in providers default to 256 outputs and operators can select another positive value or -1 for unlimited operation. Arcade operators should preserve SSE status codes and extraInfo so terminal input conflicts can be quarantined immediately. Invalid-change review now rejects an inconclusive batch without changing outputs; Monitor Admin scans by default and operators must explicitly select release for outputs positively confirmed spent. |
| `@bsv/wallet-toolbox-client`      | `2.6.5`      | `2.6.6` | patch     | [API and usage](../packages/wallet/wallet-toolbox-client.md)          | No browser consumer migration is required. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews now reject without changing outputs.                                                                                                                                                                                                                                                                                                                                                                        |
| `@bsv/wallet-toolbox-mobile`      | `2.6.5`      | `2.6.6` | patch     | [API and usage](../packages/wallet/wallet-toolbox-mobile.md)          | No mobile consumer migration is required. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews now reject without changing outputs.                                                                                                                                                                                                                                                                                                                                                                         |
...
- Release note: Follows bounded Message Box server pagination while preserving historical fetch-all behavior and adds offset/skip, total-limit, page-size, and optional page-ceiling controls.
- Migration: Existing listMessages and listMessagesLite calls continue to fetch all available messages. Applications that need an aggregate memory ceiling should set limit and/or maxPages; no additional BRC-105 approval callback is required because AuthFetch uses wallet permissions.
...
- Release note: Preserves forward-compatible browser preflights by adding every well-formed requested header to the known protocol defaults unless the operator configures a strict header allowlist.
- Migration: No consumer migration is required. Omit OVERLAY_CORS_ALLOWED_HEADERS for additive compatibility, or set an exact comma-separated list to retain a strict browser request-header policy. Origin modes, authentication, authorization, and endpoint validation are unchanged.
...
- Release note: Binds HTTPWalletWire's default browser fetch client to its global receiver so WalletClient auto-discovery can connect to Cicada on port 3301 instead of silently falling through after an Illegal invocation error.
- Migration: No consumer migration is required. Existing HTTPWalletWire constructor calls, injected HTTP clients, Wallet Wire messages, and substrate selection order remain compatible.
...
- Release note: Isolates action batches by explicit transaction-graph membership, adds bounded exact-input lease resume, makes Arcade rejection and proof reconciliation durable without an explorer, and prevents invalid-change release from treating provider failure as spent evidence.
...
- Release note: Carries the lockstep browser build with explicit action-batch isolation, bounded lease recovery, conservative Arcade rejection reconciliation, and fail-safe invalid-change review.
- Migration: No browser consumer migration is required. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews now reject without changing outputs.
...
- Release note: Carries the lockstep mobile build with explicit action-batch isolation, bounded lease recovery, conservative Arcade rejection reconciliation, and fail-safe invalid-change review.
- Migration: No mobile consumer migration is required. Related noSend chains retain local batching; unrelated actions remain on their ordinary storage path. Explorer-free Arcade status and proof recovery is automatic when Arcade is configured. Inconclusive invalid-change reviews now reject without changing outputs.
...
- Release note: Adopts the governed strict TypeScript profile and repository-wide zero-warning lint and formatting contract.
- Migration: No consumer migration is required; generated application structure and CLI behavior are unchanged.
