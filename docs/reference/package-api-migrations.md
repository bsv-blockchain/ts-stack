...
| `@bsv/overlay-topics`             | `1.6.1`      | `1.6.9` | patch     | [API and usage](../packages/overlays/overlay-topics.md)               | No consumer migration is required; topic IDs, lookup contracts, and persisted formats are unchanged. Operators of collections that already contain duplicate (txid, outputIndex) identityRecords rows should remove the duplicates so the unique index can finish building; lookups now succeed either way.                                                                                                                                                                                              |
...
- Release note: Keeps identity lookups serving when the identityRecords unique index cannot be built, retries index creation after a failed build instead of caching the rejection, and upserts identity records so repeated admission of the same output cannot create duplicate rows.
- Migration: No consumer migration is required; topic IDs, lookup contracts, and persisted formats are unchanged. Operators of collections that already contain duplicate (txid, outputIndex) identityRecords rows should remove the duplicates so the unique index can finish building; lookups now succeed either way.
