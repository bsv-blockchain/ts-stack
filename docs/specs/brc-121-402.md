---
id: spec-brc-121-402
title: BRC-121 HTTP 402 Payment Required
kind: spec
version: "1.0.0"
last_updated: "2026-04-28"
last_verified: "2026-04-28"
status: stable
tags: ["spec", "payments", "brc-121", "http-402"]
---

# BRC-121 HTTP 402 Payment Required

> BRC-121 monetizes HTTP API endpoints with a single-round-trip payment flow: client requests a resource, server responds with 402 and payment instructions (a nonce), client derives a payment address, constructs a BSV transaction, and re-sends the original request with payment headers. Server validates and serves the resource (or rejects with another 402).

## At a glance

| Field | Value |
|---|---|
| Format | OpenAPI 3.1 |
| Version | 1.0.0 |
| Status | stable |
| Implementations | @bsv/402-pay, @bsv/payment-express-middleware |

## What problem this solves

**Monetize APIs without subscriptions or token systems**. Traditional APIs require accounts, credit cards, or API keys. BRC-121 enables instant monetization: each request is micro-paid in satoshis. No signup, no account needed—just pay per request and immediate access.

**Replay-proof payment derivation**. The server provides a fresh nonce for each 402 response. The client must use that exact nonce to derive the payment address. This prevents replay attacks: a payment for one request cannot be reused for another. Additionally, the server's wallet rejects duplicate nonces with `isMerge=true`.

**Stateless server operation**. The server doesn't maintain client sessions or payment history. Every request is independent. The payment derivation is deterministic (nonce + timestamp), so the server can validate payments without database lookups.

## Protocol overview

**Single round-trip with payment**:

1. **Client → Server** `GET /protected-resource` (no payment headers)
   - Server checks if resource is free (price = 0) → serve it
   - Otherwise, server sends 402 Payment Required

2. **Server → Client** HTTP 402 response
   - `x-bsv-server: <serverPublicKey>` — Server's identity key
   - `x-bsv-sats: <amount>` — Satoshis required
   - `x-bsv-nonce: <derivationPrefix>` — Server's nonce for key derivation (BRC-29 format)

3. **Client** (constructs payment)
   - Calls `wallet.createAction()` with one P2PKH output
   - Derives address: `invoiceNumber = "2-3241645161d8-<x-bsv-nonce> <base64(timestamp))"`
   - Uses BRC-42 to derive `(pubKeyA, pubKeyB)` from sender+server keys
   - Locks output to sender's receiving address
   - Signs and constructs Atomic BEEF (BRC-95)

4. **Client → Server** same request with payment headers
   - `x-bsv-beef: <beefHex>` — Atomic BEEF transaction
   - `x-bsv-sender: <clientPublicKey>` — Sender's identity key
   - `x-bsv-nonce: <derivationPrefix>` — Same nonce from 402
   - `x-bsv-time: <base64(timestamp)>` — Timestamp suffix
   - `x-bsv-vout: <outputIndex>` — Which output contains the payment

5. **Server** (validates & serves)
   - Calls `wallet.internalizeAction()` with BEEF
   - If `isMerge === false` → payment accepted, serve resource (HTTP 200)
   - If `isMerge === true` → duplicate payment, reject (HTTP 402)
   - If amount < required → reject (HTTP 402)

## Key types / endpoints

| Method | Path | Purpose | Request | Response |
|--------|------|---------|---------|----------|
| GET/POST/etc. | `/{resourcePath}` | First request (unauthenticated) | (none) | 402 + `x-bsv-sats`, `x-bsv-nonce`, `x-bsv-server` |
| GET/POST/etc. | `/{resourcePath}` | Second request with payment | `x-bsv-beef`, `x-bsv-sender`, `x-bsv-nonce`, `x-bsv-time`, `x-bsv-vout` | 200 + resource body, or 402 on failure |

## Example: Monetized Express endpoint

```typescript
import express from 'express'
import { createAuthMiddleware } from '@bsv/auth-express-middleware'
import { createPaymentMiddleware } from '@bsv/payment-express-middleware'
import { SetupWallet } from '@bsv/wallet-toolbox'

const wallet = await SetupWallet({ env: 'main' })
const app = express()

// Stack auth, then payment middleware
app.use(createAuthMiddleware({ wallet }))
app.use(createPaymentMiddleware({
  wallet,
  calculateRequestPrice: async (req) => {
    // Pricing logic
    if (req.path === '/premium') return 500  // 500 sats
    if (req.path === '/free') return 0       // Free
    return 100  // Default 100 sats
  }
}))

// Protected endpoint
app.get('/premium', (req, res) => {
  // If we reach here, payment was accepted
  res.json({
    content: 'Premium article here',
    paidBy: req.auth.identityKey,
    amount: req.payment.satoshisPaid
  })
})

app.listen(3000)
```

Client-side (using `create402Fetch`):

```typescript
import { create402Fetch } from '@bsv/402-pay/client'
import { WalletClient } from '@bsv/sdk'

const wallet = new WalletClient()

// Fetch wrapper that auto-handles 402
const fetch402 = create402Fetch({ wallet })

const response = await fetch402('https://api.example.com/premium')
const data = await response.json()
console.log(data.content)  // Automatic payment + resource retrieval
```

## Conformance vectors

BRC-121 conformance is tested in `conformance/vectors/payments/brc121/`:

- Nonce generation and freshness (± 30 seconds)
- BRC-29 key derivation (prefix + suffix format)
- Atomic BEEF transaction validation
- `isMerge` handling (duplicate nonce rejection)
- Timestamp validation
- Output index validation

## Implementations in ts-stack

| Package | Notes |
|---------|-------|
| @bsv/402-pay/server | Server-side middleware for 402 handling; builds on payment-express-middleware |
| @bsv/402-pay/client | Client-side fetch wrapper that auto-detects 402 and constructs payment headers |
| @bsv/payment-express-middleware | Express middleware for payment gating; requires auth middleware first |

## Related specs

- [BRC-29 Peer Payment](./brc-29-peer-payment.md) — Key derivation and payment format
- [BRC-42/43](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0042.md) — Key derivation for payment address
- [BRC-95 / BRC-62](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0095.md) — Atomic BEEF format
- [BRC-31 Auth](./brc-31-auth.md) — Often combined with 402 for dual auth + payment
- [BRC-100 Wallet](./brc-100-wallet.md) — `internalizeAction()` for payment validation

## Spec artifact

[brc121.yaml](https://github.com/bsv-blockchain/ts-stack/blob/main/specs/payments/brc121.yaml)
