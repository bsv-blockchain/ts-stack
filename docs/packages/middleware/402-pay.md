...
The unpublished 0.3.3 candidate documents the BRC-121 adapter boundary. Its runtime wire contract is unchanged; BRC-118 support is provided by the SDK and composed auth/payment middleware, as described in the [payment transport guide](../../guides/brc118-payments.md).
...
- **InMemoryPaymentReplayStore** — Bounded process-local replay store used by default
...
- **PaymentReplayStore** — Atomic transaction-ID claim boundary; use a shared durable implementation across processes
...
- **Replay Protection** — Timestamp freshness + wallet validation + an independent atomic transaction claim
...
- **Atomic BEEF** — The BRC-95 subject identifies the payment transaction whose output is priced and internalized
...
3. **Transaction uniqueness** — Replaying the same BEEF is rejected by the atomic replay store; a wallet's optional `isMerge` detail is defense in depth, not the public replay contract
4. **Cache timeout too long** — If cached, same content served without payment check
5. **Atomic BEEF must be strict** — Plain BEEF and trailing bytes are rejected; unrelated branches in a legacy atomic envelope cannot influence the price
...
9. **Cluster replay state** — The default replay store is process-local; inject one shared, durable, atomic `PaymentReplayStore` into every serving process

The middleware is silent by default and uses fixed bounded wallet descriptions.
Supply its optional structured logger only when the deployment can protect
transaction diagnostics. If wallet or replay-store processing fails after
internalization starts, the middleware returns `503` without a new payment
challenge so a client is not encouraged to spend twice. `satoshisPaid` reports
the actual selected output value, including any overpayment.
...
- [Source on GitHub](https://github.com/bsv-blockchain/ts-stack/tree/main/packages/middleware/402-pay)
- [npm](https://www.npmjs.com/package/@bsv/402-pay)
