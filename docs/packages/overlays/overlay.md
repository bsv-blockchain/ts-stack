...
receipt and pending index/propagation boundary. When storage advertises a
complete `overlay-admission-v1` provider, `Engine.submit` commits through that
capability and returns the saved STEAK after majority ack. Knex storage and
its early STEAK callback do not use the capability. See the [persistence v1
...
and an explicit Engine admission adapter. Mongo is not the default storage
selection; importing `@bsv/overlay` does not load MongoDB.
...
The initial entry points are `@bsv/overlay/storage/mongo/MongoSchema`,
