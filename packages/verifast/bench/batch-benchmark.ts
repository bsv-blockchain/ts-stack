...
async function main (): Promise<void> {
  const verifier = new BdkVerifier({ maxBatchItems: 250 })
  ...
  console.log('\nPacked batch verification (milliseconds per complete batch):')
  console.log('items  Spend singles  Spend packed  speedup  EF singles  EF packed  speedup')
  for (const count of BATCH_SIZES) {
    ...
    const spendSingles = await measure(async () => {
      for (const item of spends) {
        if (!await verifier.verifySpend(item.spend)) throw new Error('Spend fixture was rejected')
      }
    })
    const spendPacked = await measure(async () => {
      if ((await verifier.verifySpendsBatch(spends)).some(valid => !valid)) throw new Error('Spend batch was rejected')
    })
    const efSingles = await measure(async () => {
      for (const item of transactions) {
        if (!await verifier.verifyScriptsFromEF(item)) throw new Error('EF fixture was rejected')
      }
    })
    const efPacked = await measure(async () => {
      if ((await verifier.verifyScriptsBatchFromEF(transactions)).some(valid => !valid)) {
        throw new Error('EF batch was rejected')
      }
    })
    console.log(
      `${String(count).padStart(5)}  ${spendSingles.toFixed(3).padStart(13)}  ` +
      `${spendPacked.toFixed(3).padStart(12)}  ${(spendSingles / spendPacked).toFixed(2).padStart(7)}x  ` +
      `${efSingles.toFixed(3).padStart(10)}  ${efPacked.toFixed(3).padStart(9)}  ` +
      `${(efSingles / efPacked).toFixed(2).padStart(7)}x`
    )
  }
