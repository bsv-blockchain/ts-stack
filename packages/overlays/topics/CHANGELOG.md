...
- **`MandalaStorageManager` extended:** new MongoDB collections `mandalaAssetStates`, `mandalaAdminHistory`, `mandalaCounters`; new methods `getAssetState`, `upsertAssetState`, `appendAdminHistory`, `getAdminHistory`, `nextAdmitSeq`; `findByAssetId` now filters evicted outpoints.
