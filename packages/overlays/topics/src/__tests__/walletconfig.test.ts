/**
 * Integration tests for WalletConfigTopicManager.
 *
 * WalletConfigTopicManager expects a PushDrop locking script with 9 fields:
 *   [configID, name, icon, wab, storage, messagebox, legal, registryOperator, signature]
 *
 * Validation:
 *   - All 8 string fields must be present
 *   - The locking public key must equal keyDeriver.derivePublicKey([1,'wallet config option'],'1',registryOperator)
 *     where keyDeriver = new KeyDeriver('anyone')
 *   - The signature must be valid: anyoneWallet.verifySignature({
 *       data: concat(fields[0..7]),
 *       signature: fields[8],
 ...
 *   - Sign data (concat of all fields except sig) with registryWallet.createSignature({counterparty:'anyone', protocolID:[1,'wallet config option'], keyID:'1'})
...
import WalletConfigTopicManager from '../walletconfig/WalletConfigTopicManager.js'
...
const PROTOCOL_ID: WalletProtocol = [1, 'wallet config option']
const KEY_ID = '1'

async function buildValidWalletConfigScript(registryPrivKey: PrivateKey): Promise<LockingScript> {
  const registryOperator = registryPrivKey.toPublicKey().toString()

  const dataFields = [
    Utils.toArray('config-001', 'utf8'),           // configID
    Utils.toArray('Test Wallet', 'utf8'),           // name
    Utils.toArray('https://example.com/icon.png', 'utf8'), // icon
    Utils.toArray('https://wab.example.com', 'utf8'),  // wab
    Utils.toArray('https://storage.example.com', 'utf8'), // storage
    Utils.toArray('https://msgbox.example.com', 'utf8'), // messagebox
    Utils.toArray('https://legal.example.com/tos', 'utf8'), // legal
...
describe('WalletConfigTopicManager', () => {
  let manager: WalletConfigTopicManager

  beforeEach(() => {
    manager = new WalletConfigTopicManager()
  })

  it('admits a valid wallet config token with all 8 fields', async () => {
    const registryPrivKey = PrivateKey.fromRandom()
    const lockingScript = await buildValidWalletConfigScript(registryPrivKey)
  ...
  it('rejects a token with missing fields (only 3 fields provided)', async () => {
    const key = PrivateKey.fromRandom()
    const f1 = Utils.toArray('config-001', 'utf8')
    const f2 = Utils.toArray('Test Wallet', 'utf8')
  ...
  it('rejects a token with an invalid signature (wrong signer)', async () => {
    ...
    const dataFields = [
      Utils.toArray('config-001', 'utf8'),
      Utils.toArray('Test Wallet', 'utf8'),
      Utils.toArray('https://example.com/icon.png', 'utf8'),
      Utils.toArray('https://wab.example.com', 'utf8'),
      Utils.toArray('https://storage.example.com', 'utf8'),
      Utils.toArray('https://msgbox.example.com', 'utf8'),
      Utils.toArray('https://legal.example.com/tos', 'utf8'),
  ...
  it('getMetaData returns expected name', async () => {
    ...
    expect(meta.name).toBe('WalletConfig')
  })
})
