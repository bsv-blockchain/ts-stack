...
class MessageBoxLookupService implements LookupService {
  ...
  async outputAdmittedByTopic(payload: OutputAdmittedByTopic): Promise<void> {
    ...
    try {
      ...
      const ad = { identityKey: Utils.toHex(identityKeyBuf), host: Utils.toUTF8(hostBuf) }
  ...
  async outputSpent(payload: OutputSpent): Promise<void> {
    ...
    if (topic === 'tm_messagebox') await this.storage.deleteRecord(txid, outputIndex)
  }
  ...
  async lookup(question: LookupQuestion): Promise<LookupFormula> {
    if (question.service !== 'ls_messagebox') throw new Error('Unsupported lookup service')
    const query = question.query as MessageBoxQuery
    if (!query?.identityKey) throw new Error('identityKey query missing')
    return await this.storage.findAdvertisements(query.identityKey, query.host)
  }

  async getDocumentation(): Promise<string> {
    return 'MessageBox Lookup Service: lookup overlay hosts for identity keys.'
  }
...
export default (mongoDb: Db): MessageBoxLookupService => new MessageBoxLookupService(new MessageBoxStorage(mongoDb))
