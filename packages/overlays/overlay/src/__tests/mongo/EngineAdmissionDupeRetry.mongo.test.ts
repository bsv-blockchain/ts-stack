...
// Same fixture BEEF used throughout the admission test suite: a single,
// unconfirmed transaction whose one input spends a proven ancestor output.
const BRC62Hex =
...
function admittingManager(): TopicManager {
  return {
    identifyAdmissibleOutputs: async (): Promise<AdmittanceInstructions> => ({
      outputsToAdmit: [0],
      coinsToRetain: []
    }),
...
function noopLookup(): LookupService {
  return {
    outputAdmittedByTopic: async () => {},
    outputSpent: async () => {},
    lookup: async () => [],
    outputNoLongerRetainedInHistory: async () => {},
    outputEvicted: async () => {},
...
describe('Engine admission retries against real MongoAdmissionStorage', () => {
