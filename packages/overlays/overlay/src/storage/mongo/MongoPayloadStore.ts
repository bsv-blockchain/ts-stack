...
export class MongoPayloadStore {
  ...
  public async publish(input: MongoPayloadInput): Promise<MongoPayloadRef> {
    ...
    const ownerId = reserved.record.ownerId
    const fence = reserved.record.fencingToken
    ...
    try {
      ...
      if (
        input.kind === 'raw-transaction' &&
        input.txid !== undefined &&
        input.txid !== streamed.txid
      )
    ...
    for (const record of stale) {
      if (record.fileId !== undefined) {
        try {
          ...
          if (recovered.modifiedCount === 1) continue
        } catch {
  ...
  private async streamInput(
    ...
    let upload: ReturnType<GridFSBucket['openUploadStreamWithId']> | undefined
    let fileId: ObjectId | undefined
    ...
    try {
      while (true) {
        ...
        if (length > BigInt(input.byteLength))
        ...
        if (inline !== undefined) {
          inline.push(chunk)
          if (length > BigInt(this.inlineCeiling)) {
            fileId = new ObjectId()
            upload = this.bucket.openUploadStreamWithId(fileId, payloadId, {
              chunkSizeBytes: this.gridFsChunkBytes,
              metadata: {
                state: 'staged',
                payloadId,
            ...
            for (const buffered of inline)
              if (!upload.write(Buffer.from(buffered)))
                await this.awaitAbort(once(upload, 'drain'), input.signal)
            inline = undefined
          }
        } else if (upload === undefined) throw new Error('Mongo payload stream writer missing')
        else if (!upload.write(Buffer.from(chunk)))
          await this.awaitAbort(once(upload, 'drain'), input.signal)
      }
      if (upload !== undefined) {
        upload.end()
        await this.awaitAbort(once(upload, 'finish'), input.signal)
      }
    } catch (error) {
      upload?.destroy(error instanceof Error ? error : new Error('Mongo payload stream failed'))
    ...
    const result: {
      ...
      inline?: Uint8Array[]
      fileId?: ObjectId
    } = { digest: digest.toString('hex'), length, inline, fileId }
  ...
  private isUint64(value: string): boolean {
    return /^(0|[1-9][0-9]{0,19})$/.test(value) && BigInt(value) <= MAX_UINT64
  }
}
