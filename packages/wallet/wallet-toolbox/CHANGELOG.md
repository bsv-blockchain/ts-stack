...
- Fix (MySQL): migration `2026-05-11-001` declared `processing_changed_at` and `observed_at` as `timestamp(3)` but defaulted to `knex.fn.now()` without precision. MySQL strict mode rejected this with `ER_INVALID_DEFAULT` because `CURRENT_TIMESTAMP` precision must match the column. Fixed by branching default on `dbtype`: `knex.fn.now(3)` for MySQL, `knex.fn.now()` elsewhere — mirrors the `addTimeStamps` pattern already used for `created_at`/`updated_at`.
- Fix (MySQL): `schemaCutover.determineDBType` mishandled `mysql2`'s `[rows, fields]` result tuple — `r[0]` returned the rows array (not the first row), so the MySQL check silently failed and the SQLite fallback was taken. The function then dispatched `PRAGMA foreign_keys = OFF` against MySQL → `ER_PARSE_ERROR`. Now unwraps `[rows, fields]`/`{rows}`/raw-array shapes uniformly, matching the robust pattern in `KnexMigrations.determineDBType`.
- Fix (cutover): legacy `transactions` rows with `txid IS NULL` (in-flight / aborted / never-broadcast) have no `transactions_new` counterpart because the backfill join keys on `txid`. The remap step would then leave `outputs`/`commissions`/`tx_labels_map` rows pointing at unmapped legacy ids and collide on `(transactionId, vout, userId)` when shifted rows landed on the orphan keyspace during the bring-down phase (`ER_DUP_ENTRY`). `runSchemaCutover` now drops these orphan FK rows before remap: `outputs.transactionId` orphans deleted, `outputs.spentBy` orphans nulled, `commissions` orphans deleted, `tx_labels_map` orphans deleted. Count is logged: `[schemaCutover] dropping FK rows for N unmapped legacy transactions (no txid)`. The legacy `transactions` rows themselves survive (renamed into `transactions_legacy`) for audit.
- Fix (MySQL): `StorageKnex.insertTransaction` and `updateTransaction` gated post-cutover routing on `isSqlite ? await this.isPostCutover() : false`. On MySQL the gate always resolved `false`, so legacy-shaped (status / userId / description / satoshis / version / ...) inserts and updates hit the new-schema `transactions` table and failed with `Unknown column 'description' in 'field list'`. The SQLite-only condition was meant to gate the FK PRAGMA toggle, not the routing decision — fix preserves SQLite-only FK toggle but routes both engines to `transactions_legacy` when post-cutover. Resolves v2-client → v3-server sync failures on `EntityTransaction.mergeNew` and `.mergeExisting` paths.
- Fix (MySQL): `StorageKnex.insertTxLabelMap` FK-bypass was SQLite-only. Post-cutover, `rebuildTxLabelsMap` repoints `tx_labels_map.transactionId` to `actions.actionId` on MySQL too (via DROP + ADD CONSTRAINT). Bridge-period sync inserts still carry legacy `transactionId` values which violated the new FK (`a foreign key constraint fails on fk_tx_labels_map_action`). Now wraps the insert in `SET FOREIGN_KEY_CHECKS=0/1` on MySQL (mirroring the SQLite `PRAGMA foreign_keys` toggle) so bridge-period imports succeed until `repointLabelsToActionId` remaps them.
- Add: `storage/index.all.ts` now re-exports `./schema/schemaCutover` so consumer apps (e.g. wallet-infra) can import `runSchemaCutover` / `rollbackSchemaCutover` directly from `@bsv/wallet-toolbox`. Previously the symbols existed but were not surfaced.
