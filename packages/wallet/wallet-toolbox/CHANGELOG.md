...
- Allow immediate actions to chain wallet-managed change from transactions
  awaiting background broadcast. The child broadcast recursively includes the
  delayed parent BEEF, preventing a large funding output from making the wallet
  appear temporarily unfunded while preserving delayed-broadcast semantics.
