import { Beef, InternalizeActionArgs, Utils } from '@bsv/sdk'
import { sdk, randomBytes, EntityProvenTxReq, StorageKnex } from '../../../src/index.all'
import { createWalletPaymentAction } from '../../../examples/walletPayment'
...
describe('walletLive test', () => {
  ...
  afterAll(async () => {
    await Promise.allSettled([stagingStorage.destroy(), myCtx.storage.destroy(), myCtx2.storage.destroy()])
  })
  ...
  test('5 pull out txid from BEEF', async () => {
    ...
    const expectedTxid = '5c574f48257202b9bff1b14baaa31cea24b9132555216900c277566d440250c5'
    const btx = beef.findTxid(expectedTxid)
...
`)
  ...
  test('6d make atomicBEEF for known txid', async () => {
    ...
    expect(Beef.fromBinary(atomic).findTxid(txid)?.txid).toBe(txid)
    console.log(`
...
`)
  ...
  test('7 test two client wallets', async () => {
    ...
    const firstUser = await myCtx.storage.findOrInsertUser(myIdentityKey)
    expect(firstUser.user.identityKey).toBe(myIdentityKey)
    ...
    const secondUser = await myCtx2.storage.findOrInsertUser(myIdentityKey2)
    expect(secondUser.user.identityKey).toBe(myIdentityKey2)
