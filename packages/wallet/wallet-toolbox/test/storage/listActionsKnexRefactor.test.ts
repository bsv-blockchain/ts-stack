/**
 * Integration tests for listActionsKnex.ts against a post-cutover SQLite
 * database.
 *
 * Verifies:
 ...
 * Knex handle (for TransactionService seeding) and the StorageKnex wrapper
 * (for calling listActions).
 */
async function setupCutoverDb (filename: string): Promise<{ knex: Knex, storage: StorageKnex }> {
  const file = await _tu.newTmpFile(filename, true, false, false)
  const knex = _tu.createLocalSQLite(file)
  const source = new KnexMigrations('test', 'listActionsKnex test', '1'.repeat(64), 1000)
...
describe('listActionsKnex — post-cutover', () => {
  ...
  // -------------------------------------------------------------------------
  // Test 2: version and lockTime are undefined for new schema rows
  // -------------------------------------------------------------------------
  test('version and lockTime are undefined for new schema rows', async () => {
    ...
    try {
      ...
      expect(action).toBeDefined()
      // schema-gap: version and lockTime not persisted; both should be undefined
  ...
  test('label filter (any) restricts results to labelled actions', async () => {
    const { knex, storage } = await setupCutoverDb('la-v7-05.sqlite')
    try {
      const svc = new TransactionService(knex)
  ...
  test('pagination returns non-overlapping pages', async () => {
    const { knex, storage } = await setupCutoverDb('la-v7-06.sqlite')
    try {
      const svc = new TransactionService(knex)
  ...
  test("sending processing status maps back to 'sending' legacy status", async () => {
    const { knex, storage } = await setupCutoverDb('la-v7-08.sqlite')
    try {
      const svc = new TransactionService(knex)
