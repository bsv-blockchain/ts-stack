...
async function migrateLatest (knex: Knex): Promise<void> {
  const source = new KnexMigrations('test', 'cutover test', '1'.repeat(64), 1000)
...
describe('the schema cutover (Knex / SQLite)', () => {
  jest.setTimeout(60_000)

  test('full legacy -> the schema cutover remaps outputs/commissions/tx_labels_map', async () => {
    const file = await _tu.newTmpFile('cutover-full.sqlite', true, false, false)
    ...
    try {
      ...
      expect(await knex.schema.hasTable('transactions_new')).toBe(false)
      ...
      const newA = await knex('transactions').where({ txid: 'a'.repeat(64) }).first('transactionId')
      const newB = await knex('transactions').where({ txid: 'b'.repeat(64) }).first('transactionId')
      expect(outs[0].transactionId).toBe(newA.transactionId)
      expect(outs[0].spentBy).toBe(newB.transactionId)
      expect(outs[1].transactionId).toBe(newA.transactionId)
      ...
      const action = await knex('actions')
        .where({ userId: 1, transactionId: newA.transactionId })
        .first('actionId')
  ...
  test('cutover on a fresh empty DB still produces final layout', async () => {
    const file = await _tu.newTmpFile('cutover-empty.sqlite', true, false, false)
    const knex = _tu.createLocalSQLite(file)
  ...
  test('cutover is idempotent on second run', async () => {
    const file = await _tu.newTmpFile('cutover-idem.sqlite', true, false, false)
    const knex = _tu.createLocalSQLite(file)
    try {
      await migrateLatest(knex)
      await seedLegacy(knex)
