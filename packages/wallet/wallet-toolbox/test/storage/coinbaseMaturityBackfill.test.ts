...
import { backfillCoinbaseMaturity } from '../../src/storage/schema/coinbaseMaturityBackfill'

/**
 * Backfill helper coverage for `outputs.matures_at_height` (schema migration
 * `2026-05-13-001`). Mirrors the `setupCutDb` pattern from
 * `schemaConformance.test.ts` so the seed data reflects a real post-cutover
 * layout.
 */
