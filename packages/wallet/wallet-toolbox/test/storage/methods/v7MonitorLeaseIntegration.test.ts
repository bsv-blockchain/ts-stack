/**
 * V7 monitor_lease integration tests.
 *
 * Tests verify the complete `V7LeasedTask` wrapper semantics end-to-end
 * against a real SQLite + V7-cutover database, including concurrent lease
 * contention and the `recordProof` flow that TaskCheckForProofs uses.
 *
 *  Test 1 — Concurrent claim: two V7TransactionService instances race for the
 ...
 *
 *  Test 5 — Pre-cutover / no-V7 gate: when getV7Service() returns undefined
...
import { V7LeasedTask } from '../../../src/monitor/V7LeasedTask'

// ---------------------------------------------------------------------------
// DB setup helper
// ---------------------------------------------------------------------------

/** Post-cutover SQLite: migrations + runV7Cutover + a single user seeded. */
...
describe('V7 monitor_lease integration (V7LeasedTask + recordProof)', () => {
  ...
  test('1: concurrent claim — only one owner wins, the other observes ran: false', async () => {
    ...
    try {
      const svcA = new V7TransactionService(knex)
      const svcB = new V7TransactionService(knex)

      const helperA = new V7LeasedTask(svcA)
      const helperB = new V7LeasedTask(svcB)
  ...
  test('2: winner calls recordProof and tx_audit has processing.changed entry for proven; loser writes nothing', async () => {
    ...
    try {
      ...
      // Seed a V7 transaction in 'sent' state (eligible for proof recording).
      // FSM allows: sent -> proven
      const txid = 'b'.repeat(64)
      const v7tx = await svcA.create({ txid, processing: 'sent' })
      ...
      if (resultA.ran) {
        ...
        const auditRows = await knex('tx_audit')
          .where({ transactionId: v7tx.transactionId, event: 'processing.changed' })
          .select('to_state')
  ...
  //
  // Strategy: claim the lease directly via tryClaimLease (bypassing V7LeasedTask
  ...
  test('3: loser succeeds on retry after winner releases', async () => {
    ...
    try {
      ...
      const helperB = new V7LeasedTask(svc)
  ...
  test('4: stale lease (expiresAt in the past) is taken over by a new claimant', async () => {
    ...
    try {
      ...
      const helper = new V7LeasedTask(svc)
      let ran = false
  ...
  // -------------------------------------------------------------------------
  // Test 5 — recordProof V7 hook: correct tx_audit entry written end-to-end.
  //
  // FSM allows: sent -> proven (see v7Fsm.ts line 22).
  // -------------------------------------------------------------------------
  test('5: recordProof via svc writes proof columns and tx_audit row', async () => {
    ...
    try {
      ...
      const v7tx = await svc.create({ txid, processing: 'sent' })
