/**
 * Integration tests for the createAction + processAction new-schema wiring.
 *
 * These tests verify:
 ...
 *  2. `TransactionService.repointLabelsToActionId()` rewrites
 *     `tx_labels_map.transactionId` from legacyTransactionId → actionId.
 *  3. The new schema row-creation block (that processAction now calls) produces the
 *     expected `transactions` + `actions` rows with correct column values.
 *
 * All tests use the `setupCutDb` helper pattern from schemaConformance.test.ts.
...
describe('createAction wiring', () => {
  ...
  test('1: insertLegacyTransaction writes to transactions_legacy on post-cutover DB', async () => {
    ...
    try {
      ...
      // Row must NOT exist in new transactions
      const inNewSchema = await knex('transactions').where('transactionId', legacyId).first()
      expect(inNewSchema).toBeUndefined()
    } finally {
      await knex.destroy()
    }
  })
  ...
  test('3: repointLabelsToActionId moves tx_labels_map.transactionId to actionId', async () => {
    ...
    try {
      ...
      // Seed a new transaction + action (first action: will be used as legacyTransactionId
      ...
      // Seed a second new transaction + action (the "real" action after signing)
  ...
  test('4: repointLabelsToActionId is a no-op when no labels point to legacyTransactionId', async () => {
    ...
    try {
      ...
      // Seed a new transaction + action (no label rows)
  ...
  // -----------------------------------------------------------------------
  // Test 5 — findOrCreateActionForTxid (processAction core) creates both rows
  // -----------------------------------------------------------------------
  test('5: findOrCreateActionForTxid creates new transactions + actions rows', async () => {
    ...
    try {
      ...
      // new transactions row exists with correct fields
      ...
      // new actions row exists with correct fields
  ...
  // -----------------------------------------------------------------------
  // Test 6 — Full processAction new-schema wiring simulation:
  //   legacy row in transactions_legacy → processAction new-schema block creates new schema rows
  //   + rewrites tx_labels_map to actionId
  // -----------------------------------------------------------------------
  test('6: processAction new-schema block creates new schema rows and repoints labels', async () => {
    ...
    try {
      ...
      // Verify legacy row exists and new transactions table is empty
      const inLegacy = await knex('transactions_legacy').where({ transactionId: legacyId }).first()
      expect(inLegacy).toBeDefined()
      expect(await knex('transactions').count<{ c: number }>({ c: '*' }).first().then(r => Number(r!.c))).toBe(0)

      // 2. Simulate what processAction's new-schema block does (after signing produces a txid).
      ...
      // new transactions row exists
      const newTxRow = await knex('transactions').where({ txid: realTxid }).first()
      expect(newTxRow).toBeDefined()
      expect(newTxRow.processing).toBe('queued')

      // new actions row exists for userId=1
      const newAction = await knex('actions').where({ actionId: action.actionId }).first()
      expect(newAction).toBeDefined()
      expect(newAction.userId).toBe(1)
      expect(newAction.reference).toBe(legacyRef)
      ...
      // Legacy row still exists in transactions_legacy (untouched by new-schema wiring)
