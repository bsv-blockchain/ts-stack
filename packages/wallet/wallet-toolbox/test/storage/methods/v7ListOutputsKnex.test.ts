/**
 * V7 post-cutover integration tests for listOutputsKnex.ts
 *
 * These tests verify the two fixes introduced in the V7 wiring:
 ...
 *
 * Setup pattern mirrors v7Conformance.test.ts and v7ServiceExpansion.test.ts.
 */
...
/**
 * Creates a fresh post-V7-cutover SQLite database, runs all migrations, then
 * executes `runV7Cutover`. Two users are pre-seeded (userId 1 and 2).
 */
...
describe('listOutputsKnex V7 — status filter + label join', () => {
  ...
  test('returns output with correct labels when transaction is proven', async () => {
    ...
    try {
      ...
      // Create a V7 transaction in `proven` state.
  ...
  test('does NOT return outputs for transactions in queued state', async () => {
    ...
    try {
      ...
      // Create a V7 transaction in `queued` state (not yet broadcast).
  ...
  test('returns outputs for proven/sent/seen/nosend/sending but not queued/invalid', async () => {
    ...
    try {
      ...
      for (const s of states) {
        // Insert raw row: bypass V7TransactionService.create to avoid FSM
        // constraints on terminal states (invalid / doubleSpend).
        // V7 transactions table uses snake_case column names.
