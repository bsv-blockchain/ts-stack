...
import { sdk, StorageProvider, verifyOne } from '../../src/index.all'
...
import {
  ...
  expectReconcilableSourceOutputUnchanged,
  seedFailedTransactionOutputState,
  seedReconcilableSourceOutputState
...
describe('update tests', () => {
  ...
  test('17_reviewStatus does not restore outputs generated by a failed-but-reconcilable transaction', async () => {
    for (const { storage } of setups) {
      const state = await seedReconcilableSourceOutputState(storage)

      await storage.reviewStatus({ agedLimit: new Date() })
      await expectReconcilableSourceOutputUnchanged(storage, state)
    }
  })
})
