...
describe('V7 CRUD + audit (Knex / SQLite)', () => {
  ...
  beforeAll(async () => {
    const file = await _tu.newTmpFile('v7crud.sqlite', true, false, false)
  ...
  test('insert + findByTxid + findById round-trip', async () => {
    const id = await insertTransactionV7(knex, {
      txid: 'a'.repeat(64),
      processing: 'queued',
    ...
    const byTxid = await findTransactionV7ByTxid(knex, 'a'.repeat(64))
    expect(byTxid?.processing).toBe('queued')
    const byId = await findTransactionV7(knex, id)
    expect(byId?.txid).toBe('a'.repeat(64))
  })

  test('transitionProcessing performs CAS + writes audit row on success', async () => {
    const row = await findTransactionV7ByTxid(knex, 'a'.repeat(64))
