...
import { runKnexBackfill } from '../../src/storage/schema/backfill.knex'

describe('the backfill (Knex / SQLite)', () => {
  jest.setTimeout(60_000)

  let knex: Knex

  beforeAll(async () => {
    const file = await _tu.newTmpFile('backfill.sqlite', true, false, false)
    knex = _tu.createLocalSQLite(file)
    const source = new KnexMigrations('test', 'backfill test', '1'.repeat(64), 1000)
    ...
    const names = (await source.getMigrations()).filter(
      n => n !== '2026-05-12-001 the schema cutover (remap FKs, rename legacy tables, swap transactions_new)'
    )
  ...
  test('backfills transactions_new and actions from legacy rows', async () => {
    ...
    const stats = await runKnexBackfill(knex, new Date('2026-05-11T12:00:00Z'))
    ...
    const newA = await knex('transactions_new').where({ txid: txidA }).first()
    expect(a.transactionId).toBe(newA.transactionId)
  })

  test('is idempotent on a second run', async () => {
    const before = await knex('transactions_new').count<{ c: number }>({ c: '*' }).first()
    const beforeActions = await knex('actions').count<{ c: number }>({ c: '*' }).first()
    await runKnexBackfill(knex)
    const after = await knex('transactions_new').count<{ c: number }>({ c: '*' }).first()
