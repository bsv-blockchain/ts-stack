...
Multi-week phased production rollout of the V7 schema migration (per `PROD_REQ_V7_TS.md`). This is a **project plan**, not an operator runbook. For step-by-step cutover mechanics, see `docs/V7_CUTOVER_RUNBOOK.md`.
...
|---|---|---|---|---|
| **Cutover failure mid-rename** | Medium | High — torn FK state, writer pool stalled | `runV7Cutover` is idempotent; `transactions_legacy` presence guard recovers from partial failure. Always run a staging dry-run on a production snapshot first. Maintenance-mode window 2× expected duration. | SRE on-call |
...
| **Monitor race during cutover** | Low | High — chain tip writes mid-rename | Stop Monitor daemon before runV7Cutover (runbook §2.2). Confirm `monitor_lease` is empty before proceeding. | SRE on-call |
...
| **IDB client cutover lag** | Medium | Medium — mobile clients on stale schema | `runV7IdbCutover` runs on next app open. Add telemetry to count clients still on legacy schema; force-upgrade prompt at 14 days. | Mobile Eng |
...
| Day | Activity |
|---|---|
| Mon | Cut release branch. Deploy V7 additive code (creates `transactions_v7`, `actions`, `outputs` parallel structure) to staging. |
| Tue | Run V7 migration in additive mode on staging snapshot of production data. Capture row counts, durations, query plans. |
| Wed | Shadow-run V7TransactionService writes against staging — both schemas updated, reads still hit legacy. Verify divergence metric stays at 0. |
| Thu | Deploy V7 additive code to production (no cutover). Verify zero impact on hot query latency. |
...
| Mon | Execute `runV7Cutover` on staging. Execute `runV7IdbCutover` against staging IDB fixtures. Time the cutover; record duration. |
...
| Mon (window) | Scheduled maintenance window — 02:00–04:00 UTC. Drain writers. Stop Monitor. Run `runV7Cutover` on production. Run smoke tests. Reopen writers on green. |
...
**Exit criteria:** Conformance suite green on day 7; zero P1/P2 production incidents; orphan checks return 0; performance within 5% of week-1 baseline; >95% of mobile clients on V7 IDB schema.
...
2. V7 additive migration applied: `SELECT name FROM sqlite_master WHERE type='table' AND name='transactions_v7'` returns one row.
3. Deployed application code is on the V7 storage path (`V7TransactionService` enabled, no callers reference `proven_txs` or `proven_tx_reqs`).
...
- **NO-GO:** Any open incident referencing V7 tables; IDB migration <95%; any caller still referencing `*_legacy`.
...
| Cutover duration histogram | runV7Cutover step timings | <budget per runbook §1.5 | Real-time during cutover only |
...
| Shadow-write divergence (week 1) | App-side comparator: legacy vs. v7 row state | Always 0 | Page on first nonzero |
...
**Dashboards required:**
- **V7 Rollout** — all metrics above, single page, refresh 30s.
- **V7 Performance** — hot query latencies vs. baseline, 1h/24h/7d.
- **V7 Audit** — tx_audit event volume by type, FSM transition violations, orphan check history.
...
**OPL-XXX — V7 Schema Rollout to Production**
...
**OPL-XXX-1 — Deploy V7 additive code to staging**
- Labels: `staging`, `cutover`
- Acceptance: `transactions_v7`, `actions`, `outputs` tables exist in staging; row counts match expectations; no impact on staging query latency for 24h.
...
**OPL-XXX-3 — Deploy V7 additive code to production**
- Labels: `cutover`
- Acceptance: Production has `transactions_v7` + `actions` tables; zero impact on hot query latency for 24h post-deploy; rollback path validated.

**OPL-XXX-4 — Performance baseline capture**
- Labels: `observability`
- Acceptance: p50/p95/p99 of `findSpendableOutputs`, `refreshOutputsSpendable`, monitor tick duration documented in V7 Performance dashboard. Numbers signed off by Performance Eng.

#### Week 2 — Staging cutover

**OPL-XXX-5 — Build V7 Rollout dashboard**
- Labels: `observability`
...
- Labels: `staging`, `cutover`
- Acceptance: `runV7Cutover` executed on staging restored from prod snapshot; duration measured; smoke tests pass; results documented.

**OPL-XXX-7 — Run §5 conformance suite against post-cutover staging**
- Labels: `staging`
- Acceptance: All conformance tests (existing + new V7 tests) green on day 1, day 3, day 7 of staging soak.
...
- Acceptance: SRE on-call restores from a recent prod backup on a scratch host; documents restore duration; confirms `rollbackV7Cutover` works on the rehearsal data.
...
- Acceptance: Daily metrics review for 7 days; all thresholds in §6 stayed within bounds; orphan checks 0; no V7-attributable incidents.
