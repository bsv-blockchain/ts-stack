...
import { appendTxAudit, auditProcessingTransition } from './v7TxAudit'
...
export class V7TransactionService {
  ...
  async findActionByReference (
    userId: number,
    reference: string
  ): Promise<{ action: TableAction, transaction: TableTransactionV7 } | undefined> {
  ...
  async findOrCreateActionForTxid (args: {
    ...
    processing?: sdk.ProcessingStatus
    now?: Date
  }): Promise<{ action: TableAction, transaction: TableTransactionV7, isNew: boolean }> {
    const now = args.now ?? new Date()
  ...
  async createWithProof (args: {
    ...
    const stored = await this.findById(id)
    if (stored == null) throw new Error(`V7 transaction ${id} disappeared after createWithProof insert`)
    return stored
  }
  ...
  async findOrCreateForBroadcast (args: {
    ...
    processing?: sdk.ProcessingStatus
    now?: Date
  }): Promise<{ transaction: TableTransactionV7, isNew: boolean }> {
