import { V7TransactionService } from '../storage/schema/v7Service'

/**
 * Helper that wraps a long-running monitor task body in V7 `monitor_lease`
 ...
 */
export class V7LeasedTask {
  constructor (private readonly svc: V7TransactionService) {}
  ...
  async run (
    ...
    if (!claim.acquired) {
      console.log(`[V7LeasedTask] ${taskName}: lease held by another owner — skipping`)
      return { ran: false }
    }

    console.log(`[V7LeasedTask] ${taskName}: lease acquired by ${ownerId} (ttl=${ttlMs}ms)`)
    ...
    const renewTimer = setInterval(async () => {
      try {
        ...
        if (result.acquired) {
          renewFailures = 0
          console.log(`[V7LeasedTask] ${taskName}: lease renewed (renewCount=${result.lease?.renewCount ?? '?'})`)
        } else {
          renewFailures++
          console.log(`[V7LeasedTask] ${taskName}: lease renew failed (attempt ${renewFailures}) — may have been taken over`)
        }
      } catch (err: unknown) {
        renewFailures++
        const msg = err instanceof Error ? err.message : String(err)
        console.log(`[V7LeasedTask] ${taskName}: lease renew error (attempt ${renewFailures}): ${msg}`)
      }
    }, renewIntervalMs)
    ...
    try {
      ...
      try {
        ...
        console.log(`[V7LeasedTask] ${taskName}: lease released by ${ownerId} (deleted=${released})`)
      } catch (releaseErr: unknown) {
        const msg = releaseErr instanceof Error ? releaseErr.message : String(releaseErr)
        console.log(`[V7LeasedTask] ${taskName}: lease release error: ${msg}`)
      }
    }
  }
}
