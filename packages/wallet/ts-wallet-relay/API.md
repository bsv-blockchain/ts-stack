...
| `signQrCodes`           | `boolean`                         | No       | `true`                                          | Sign the QR pairing URI with the backend wallet key. Secure mobile clients require this signature and reject unsigned pairing URIs. `false` exists only for explicitly managed legacy migrations and is incompatible with `WalletPairingSession.resolveRelay()`.                                                                                                                                                                                                                                                                                                                                                                          |
...
| `onSocketClosed`        | `(info: SocketCloseInfo) => void` | No       | —                                               | Called for every accepted WebSocket that closes, both roles, with the close code, the cause (`client`, `heartbeat` or `server`) and how long it was open. Fires before the session bookkeeping that drives `onSessionDisconnected`. Intended for logging, so a dropped phone can be diagnosed from server logs. Thrown errors and rejected promises are contained so logging cannot prevent cleanup.                                                                                                                                                                                                                                      |
| `heartbeatIntervalMs`   | `number`                          | No       | `30000`                                         | How often to ping every socket, in milliseconds. Must be an integer from 1 to 2147483647; forwarded to `WebSocketRelay`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `maxMissedHeartbeats`   | `number`                          | No       | `2`                                             | Consecutive missed pongs tolerated before a socket is terminated. Must be an integer from 1 to 2147483647; set `1` for the pre-0.5 terminate-on-first-miss behaviour. Forwarded to `WebSocketRelay`.                                                                                                                                                                                                                                                                                                                                                                                                                                      |     |
...
| ----------------------- | ---------------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apiUrl`                | `string`                           | `'/api'`                          | Base URL for the relay HTTP API. Absolute URLs require HTTPS except on loopback; root-relative same-origin paths are accepted. Credentials, query strings, and fragments are rejected. The `/api` suffix is appended automatically if missing. |
...
| `onApprovalRequired` | `(method, params) => Promise<boolean>` | `undefined`                    | Called for every implemented method not in `autoApproveMethods`. Return `true` to approve, `false` to send a `4001 User Rejected` response. If omitted, methods outside `autoApproveMethods` fail closed with `4001`; they are never auto-approved.                                                                                                                                                                                                                                                    |
...
| Option                  | Type      | Default                           | Description                                                                                                                                                                                                                                                                                                 |
| ----------------------- | --------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apiUrl`                | `string`  | `'/api'`                          | Backend base URL. Absolute URLs require HTTPS except on loopback; root-relative paths are accepted. `/api` is appended automatically if missing.                                                                                                                                                            |
...
- Heartbeat: pings every `heartbeatIntervalMs` (default 30 s); a socket is terminated once it has missed `maxMissedHeartbeats` (default 2) consecutive pongs without sending anything, so a quiet socket lives between two and three intervals. Any inbound message also counts as proof of life.
- Every accepted socket close is reported through `onSocketClose` with its code, cause and duration
...
interface WebSocketRelayOptions {
  ...
  heartbeatIntervalMs?: number      // ping period in ms, integer 1–2147483647 (default 30000)
  maxMissedHeartbeats?: number      // consecutive missed pongs tolerated, integer >= 1 (default 2)
}

Both heartbeat options throw `RangeError` when given anything other than an integer >= 1. `NaN` and `Infinity` are rejected rather than clamped, because Node coerces either to a 1 ms timer.
```
...
```ts
onDisconnect(handler: (topic: string, role: 'desktop' | 'mobile', info: SocketCloseInfo) => void): void
```

Called when a socket that still held its topic slot closes. Use this to react to mobile disconnects — for example, to immediately reject in-flight RPC requests rather than waiting for the 30-second timeout. The third argument is additive; existing two-argument handlers keep working.
...
```ts
onSocketClose(handler: (info: SocketCloseInfo) => void): void
```

Called for every accepted socket that closes, whether or not it still held its topic slot. Unlike `onDisconnect` this also fires for a socket that was replaced by a newer connection on the same topic and role. Intended for logging. Thrown errors and rejected promises are contained; callback completion is not awaited and cannot prevent disconnect cleanup.
...
| Origin scheme       | Must be `https://`; `http://` is allowed only for loopback development hosts          |
...
| sig                 | Parsed when present; `WalletPairingSession.resolveRelay()` requires a valid signature |
...
Verifies the `sig` field embedded in a parsed `PairingParams` object. Returns
`false` when `params.sig` is absent, invalid, or the signed fields have been
tampered with. `WalletPairingSession.resolveRelay()` performs this verification
automatically before making any network request.
...
One record per accepted socket close, delivered to `onSocketClose` (and as the third argument to `onDisconnect`). Intended for logging.

```ts
type SocketCloseCause = 'client' | 'heartbeat' | 'server'

interface SocketCloseInfo {
  topic: string
  role: 'desktop' | 'mobile'
  code: number // 1006 for an abnormal drop, 1005 when the peer sent none
  reason: string // empty when none was sent
  cause: SocketCloseCause
  connectedForMs: number // between the upgrade being accepted and the close event
  missedPongs: number // consecutive missed pongs at the moment of close
}
```

`cause` distinguishes `'client'` (the peer closed it, or the transport dropped) from `'heartbeat'` (this relay terminated it for missing too many pongs) and `'server'` (this relay closed it deliberately — auth timeout, proof failure, `deleteSession`).
