...
// ─── onDataError: failures after the HTTP exchange ───────────────────────────

describe('SimplifiedFetchTransport onDataError', () => {
  test('reports a message-processing failure with the message that caused it', async () => {
    const transport = new SimplifiedFetchTransport('https://api.example.com', jest.fn() as any)
    const failure = new Error('Session not found for nonce: other-session')
    const reported: Array<{ error: Error, message: AuthMessage }> = []

    transport.onDataError((error, message) => { reported.push({ error, message }) })
    await transport.onData(async () => { throw failure })

    const message = makeAuthMessage('general')
    ;(transport as any).onDataCallback(message)
    await new Promise<void>(resolve => setTimeout(resolve, 0))
  ...
  test('wraps a non-Error failure before reporting it', async () => {
    const transport = new SimplifiedFetchTransport('https://api.example.com', jest.fn() as any)
    const reported: Error[] = []

    transport.onDataError((error) => { reported.push(error) })
    await transport.onData(async () => { throw 'session gone' })
  ...
  test('swallows the failure when no error listener is registered', async () => {
    ...
    await transport.onData(async () => { throw new Error('processing failed') })

    expect(() => { (transport as any).onDataCallback(makeAuthMessage('general')) }).not.toThrow()
    await new Promise<void>(resolve => setTimeout(resolve, 0))
