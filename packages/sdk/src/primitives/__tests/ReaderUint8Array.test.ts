...
describe('ReaderUint8Array', () => {
  ...
  it('should be able to read 0 bytes', () => {
    ...
    expect(new ReaderUint8Array([...buf]).read(0)).toHaveLength(0)
  })
})
...
describe('#readReverse', () => {
  ...
  it('should be able to read 0 bytes', () => {
    const buf = Buffer.from('0101', 'hex')
    expect(new ReaderUint8Array([...buf]).readReverse(0)).toHaveLength(0)
  })
})
...
describe('#readInt32LE', () => {
  it('should return 1', () => {
    ...
    expect(new ReaderUint8Array([...Buffer.from('ffffffff', 'hex')]).readInt32LE()).toEqual(-1)
  })
})
