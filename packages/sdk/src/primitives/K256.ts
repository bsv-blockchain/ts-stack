...
export default class K256 extends Mersenne {
  ...
  override split(input: BigNumber, output: BigNumber): void {
    ...
    const tempOutputWords = Array.from(
      { length: outLen + (inputNominalLength > 9 ? 1 : 0) },
      () => 0
    )
    ...
    if (currentInputNewWordCount < tempInputNewWords.length) {
      ...
      // If prev is non-zero but no space, this is an issue.
  ...
  override imulK(num: BigNumber): BigNumber {
    ...
    for (let i = 0; i < newNominalLength; i++) {
      // Iterate up to new expanded length
