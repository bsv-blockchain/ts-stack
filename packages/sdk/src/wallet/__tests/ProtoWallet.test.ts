...
describe('ProtoWallet', () => {
  it('Throws when unsupported functions are called', async () => {
    ...
    expect(() =>
      (wallet as unknown as { nonExistentFunction: () => void }).nonExistentFunction()
    ).toThrow()

    // Example: Ensure it throws when calling a method incorrectly
    expect(() =>
      (wallet as unknown as { derivePrivateKey: () => void }).derivePrivateKey()
    ).toThrow()
  })
  ...
  it('Validates the BRC-3 compliance vector', async () => {
    ...
    const { valid } = await wallet.verifySignature({
      ...
      signature: [
        ...
        198, 201, 21, 56, 82, 141, 112, 69, 196, 170, 73, 156, 6, 44, 48, 2, 32, 118, 125, 254, 201,
        44, 87, 177, 170, 93, 11, 193, 134, 18, 70, 9, 31, 234, 27, 170, 177, 54, 96, 181, 140, 166,
  ...
  it('Validates the BRC-2 HMAC compliance vector', async () => {
    const wallet = new ProtoWallet(
      new PrivateKey('6a2991c9de20e38b31d7ea147bf55f5039e4bbc073160f5e0d541d1f17e321b8', 'hex')
    )
