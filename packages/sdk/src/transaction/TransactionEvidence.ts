...
function isInvalidEvidenceByte(byte: number): boolean {
  return !Number.isInteger(byte) || byte < 0 || byte > 255
}

function snapshotEvidenceBytes(
  ...
  if (bytes.length === 0 || bytes.some(isInvalidEvidenceByte)) {
    throw new TransactionEvidenceError('invalid-evidence')
  }
  return bytes
}

function loadEvidenceTransaction(
  bytes: number[],
  limits: TransactionEvidenceLimits,
...
function graphScriptBytes(tx: Transaction): number {
  let scriptBytes = 0
  for (const output of tx.outputs) scriptBytes += output.lockingScript.toBinary().length
  for (const input of tx.inputs) scriptBytes += input.unlockingScript?.toBinary().length ?? 0
  return scriptBytes
}

function enqueueUnconfirmedParents(
  current: Transaction,
  pending: Transaction[],
  spent: Set<string>
): void {
...
function assertUnconfirmedGraph(tx: Transaction, limits: TransactionEvidenceLimits): void {
  ...
  while (pending.length > 0) {
    ...
    visited.add(id)
    scriptBytes += graphScriptBytes(current)
    ...
    enqueueUnconfirmedParents(current, pending, spent)
  }
}
