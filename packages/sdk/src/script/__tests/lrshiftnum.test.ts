  ...
  test('1 invalid OP_LSHIFTNUM Scripts', async () => {
    for (const [script, stack] of lshiftnumInvalidScripts) {
      try {
        ...
        const _result = executeScriptCode(script, stack)
...
function executeScriptCode(script: Script | string, expectedStack: string[]): boolean {
  if (typeof script === 'string') script = Script.fromASM(script)
  ...
  for (let i = 0; i < script.chunks.length; i++) spend.step()
  const stack = spend.stack
  if (stack.length !== expectedStack.length) {
    console.log(
      `script ${script.toASM()} expected stack length ${expectedStack.length} but got ${stack.length}`
    )
    return false
  }
