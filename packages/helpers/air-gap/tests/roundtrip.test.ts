...
import { drain, lcg, message, shuffled } from './helpers'
...
describe('round trip', () => {
  ...
  it('needs only a small overhead over K parts on average', () => {
    ...
    // finishes in barely more than K reads.
  ...
  it('never blends two senders in view of the same camera', () => {
    ...
    const encA = new AirGapEncoder(msgA, 1200)
    const encB = new AirGapEncoder(msgB, 1200)
    const dec = new AirGapDecoder()
    // Alternating parts from two different messages starve each other — each
    // one resets the session the other was building — but nothing is ever
    // emitted from the mixture.
    for (let seq = 0; seq < 40; seq++) {
      expect(dec.accept(encA.partAt(seq)).done).toBe(false)
      expect(dec.accept(encB.partAt(seq)).done).toBe(false)
      expect(dec.message()).toBeNull()
    }
    // Point the camera at one of them and it decodes immediately.
    expect(drain(dec, encA, [0, 1])).not.toBeNull()
