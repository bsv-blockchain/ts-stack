...
export function installProject (config: ProjectConfig, targetDir: string, runCommand: RunCommand): string[] {
  ...
  if (getStarter(config.starter)?.kind === 'repository' && existsSync(join(targetDir, 'package.json'))) candidates.unshift(targetDir)
  if (candidates.length === 0 && existsSync(join(targetDir, 'package.json'))) candidates.push(targetDir)
