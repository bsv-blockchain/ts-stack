...
export function viteCommand (pm: PackageManager, dir: string, variant: string): { command: string, args: string[], cwd: string } {
  ...
  if (pm === 'npm') return { command: 'npm', args: ['create', `vite@${CREATE_VITE_VERSION}`, name, '--', '--template', variant, '--eslint'], cwd }
  if (pm === 'yarn') return { command: 'yarn', args: ['create', `vite@${CREATE_VITE_VERSION}`, name, '--template', variant, '--eslint'], cwd }
  // pnpm and bun
  return { command: pm, args: ['create', `vite@${CREATE_VITE_VERSION}`, name, '--template', variant, '--eslint'], cwd }
}
