...
export type TargetSpec =
  | { kind: 'frontend', target: FrontendTarget }
  | { kind: 'backend', target: BackendTarget }

export type RunCommand = (command: string, args: string[], opts: { cwd: string }) => void

export interface BaseScaffolder {
  scaffold: (spec: TargetSpec, absDir: string, opts: { packageManager: PackageManager, runCommand: RunCommand }) => void
}
