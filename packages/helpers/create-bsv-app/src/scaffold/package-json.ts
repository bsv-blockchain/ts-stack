...
export function applyCapabilityDeps (targetDir: string, deps: Record<TargetKey, Record<string, string>>): void {
  const dirForTarget = (key: TargetKey): string => (key === 'root' ? targetDir : join(targetDir, key))
