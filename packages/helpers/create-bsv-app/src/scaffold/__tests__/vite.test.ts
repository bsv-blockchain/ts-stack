...
describe('viteCommand', () => {
  test('npm uses the -- separator before --template and --eslint, run from the parent dir', () => {
    expect(viteCommand('npm', '/proj/client', 'react-ts')).toEqual({
      command: 'npm', args: ['create', 'vite@latest', 'client', '--', '--template', 'react-ts', '--eslint'], cwd: '/proj'
    })
  })
  test('pnpm/yarn/bun omit the -- separator and pass --eslint', () => {
    expect(viteCommand('pnpm', '/proj/client', 'react-ts').args).toEqual(['create', 'vite@latest', 'client', '--template', 'react-ts', '--eslint'])
    expect(viteCommand('yarn', '/proj/client', 'react-ts').args).toEqual(['create', 'vite', 'client', '--template', 'react-ts', '--eslint'])
    expect(viteCommand('bun', '/proj/client', 'react-ts').args).toEqual(['create', 'vite@latest', 'client', '--template', 'react-ts', '--eslint'])
  })
})
...
describe('viteScaffolder', () => {
  test('invokes the injected runCommand with the computed vite command', () => {
    ...
    expect(calls[0]).toEqual({ command: 'npm', args: ['create', 'vite@latest', 'client', '--', '--template', 'react-ts', '--eslint'], cwd: '/proj' })
  })
})
