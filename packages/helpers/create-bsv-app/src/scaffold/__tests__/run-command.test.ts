...
test('runs with stdin NOT inherited so interactive generators stay non-interactive', () => {
  ...
  const run = makeRunCommand((_c, _a, options) => { captured = options; return { status: 0 } })
...
test('forwards command and args verbatim', () => {
  ...
  const run = makeRunCommand((command, args) => { seen.push({ command, args }); return { status: 0 } })
  run('pnpm', ['create', 'vite@latest', 'app', '--template', 'react-ts'], { cwd: '.' })
  expect(seen).toEqual([{ command: 'pnpm', args: ['create', 'vite@latest', 'app', '--template', 'react-ts'] }])
})
