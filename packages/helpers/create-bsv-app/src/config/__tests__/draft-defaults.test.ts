...
jest.mock('../../registry', () => ({
  listCapabilities: () => ([
    { id: 'wallet-connect', defaultSelected: true, roles: [], files: () => ({}), npmDependencies: () => ({}), agentsSection: () => '' },
    { id: 'extra', roles: [], files: () => ({}), npmDependencies: () => ({}), agentsSection: () => '' }
  ])
}))
...
describe('seedDraft default-selected capabilities', () => {
  ...
  test('NEW mode unions explicit flags with defaults', () => {
    expect(seedDraft(null, { capabilities: ['extra'] }).capabilities?.sort()).toEqual(['extra', 'wallet-connect'])
  })
  test('ADD mode does NOT auto-select defaults', () => {
    const m = { version: 1 as const, name: 'x', network: 'test' as const, stack: {}, bsvDir: 'src/bsv', capabilities: [] }
