...
describe('SocketServerTransport', () => {
  ...
  ])('contains %s and disconnects only the offending socket', async (_label, failure) => {
    const socket = createMockSocket()
    const onError = jest.fn().mockRejectedValue(new Error('observer failed'))
    const transport = new SocketServerTransport(socket as never, { onError })
  ...
  test('disconnects a socket that exceeds its authentication concurrency limit', async () => {
    const socket = createMockSocket()
  ...
  test('releases authentication capacity after each successful message', async () => {
    const socket = createMockSocket()
    const transport = new SocketServerTransport(socket as never, { maxPendingMessages: 1 })
  ...
  test('rejects invalid concurrency limits', () => {
    const socket = createMockSocket()
    expect(() => new SocketServerTransport(socket as never, { maxPendingMessages: 0 })).toThrow(
