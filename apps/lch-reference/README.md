...
The creator wizard encrypts a local asset into authenticated segments, stores detached ciphertext through a `ContentSink`, signs an inline Offer, and declares a two-wallet compensation split. The player performs a non-spending preflight, obtains a signed Quote, Payment Demands, and short-lived signed Payment Readiness leases, and stops at an explicit confirmation boundary. Confirmation refreshes readiness and asks a BRC-100 `WalletInterface` to create one BRC-105/BRC-29 transaction. Each Payee has its own signed delivery endpoint, independently validates and internalizes its output, and returns a signed Receipt to the buyer for issuer completion. The issuer delivers BRC-78 key grants only after all receipts match; the player verifies license recovery, decrypts the bytes, and renders the media. A delivery outage leaves a visible pending-settlement record with finalized transaction bytes, the exact partial Receipt count, and the recovery deadline; finalized bytes are not mislabeled as broadcast or mined.

The default browser workbench uses deterministic fixture wallets. They execute the same wallet methods and cryptographic derivations as the connected flow while constructing an input-free Atomic BEEF fixture. The collapsed server still routes issuer, recording-controller, and composition-controller messages through distinct endpoints and rejects a Demand sent to the wrong Payee route. Package tests also run those roles at three distinct HTTPS origins. The Node reference server accepts an operator-provided wallet module, where each returned `WalletInterface` can be backed by a real BRC-100 wallet service. See [DEPLOYMENT.md](./DEPLOYMENT.md).
