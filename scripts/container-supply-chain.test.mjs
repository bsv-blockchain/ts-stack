...
test('container release workflows scan before push and publish signed evidence', () => {
  ...
  assert.match(infraRelease, /all-matrix: \$\{\{ steps\.set\.outputs\.all-matrix \}\}/)
  assert.match(infraRelease, /verify-published:/)
  assert.match(infraRelease, /GH_TOKEN: \$\{\{ github\.token \}\}/)
  assert.match(infraRelease, /needs\.verify-published\.result == 'success'/)
  assert.match(infraRelease, /org\.opencontainers\.image\.revision/)
  assert.match(infraRelease, /platform" != "linux\/amd64"/)
