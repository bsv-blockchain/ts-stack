...
test('current required, manual, live, resource, and conformance tests are governed', () => {
  ...
  assert.equal(result.summary.propertySuites, 31)
  assert.equal(result.summary.propertyPackages, 29)
  assert.equal(result.summary.propertyExcludedPackages, 6)
  assert.equal(result.summary.propertyClassifiedPackages, 35)
  assert.equal(result.summary.mutationTargets, 31)
